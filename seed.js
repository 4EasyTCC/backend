...
console.log("Tabelas apagadas com sucesso.");
    ...
    // 2. Criar localizações (criação sequencial para garantir IDs)
    ...
    for (const loc of localidadesData) {
      const created = await Localizacao.create(loc);
      localizacoes.push(created);
    }
    console.log("Localizações criadas com sucesso.");

    // 3. Criar eventos sequencialmente (para garantir que temos eventoId)
    console.log("Criando eventos...");
    const eventosData = [
      {
        nomeEvento: "Festival de Música Verão 2025",
        descEvento:
          "Um incrível festival com as melhores bandas nacionais e internacionais na bela praia de Copacabana.",
        ...
        horaFim: "02:00:00",
        statusEvento: "ativo",
        localizacaoIndex: 2, // Rio de Janeiro
      },
      {
        nomeEvento: "Workshop de Gastronomia Italiana",
        descEvento:
          "Aprenda a fazer massas e molhos autênticos da Itália com chefs renomados, no coração de São Paulo.",
        ...
        horaFim: "18:00:00",
        statusEvento: "ativo",
        localizacaoIndex: 0, // São Paulo
      },
      {
        nomeEvento: "Maratona de São Paulo 2026",
        descEvento:
          "Participe da maior corrida de rua da América Latina, com percurso de 42km pelas principais ruas da cidade.",
        categoria: "Esporte",
        privacidadeEvento: "Público",
    ...
    for (const ev of eventosData) {
      const eventoCriado = await Evento.create({
        ...
        descEvento: ev.descEvento,
        categoria: ev.categoria,
        privacidadeEvento: ev.privacidadeEvento,
        dataInicio: ev.dataInicio,
        horaInicio: ev.horaInicio,
        dataFim: ev.dataFim,
        horaFim: ev.horaFim,
        statusEvento: ev.statusEvento,
        localizacaoId: localizacoes[ev.localizacaoIndex].localizacaoId,
    ...
    for (let i = 0; i < eventos.length; i++) {
      await Midia.create({
        ...
        url: i === 2 ? "/uploads/evento3.jpeg" : `/uploads/evento${i + 1}.jpg`,
        tipo: "capa",
      });
    }
    console.log("Mídias adicionadas com sucesso.");

    // 5. Adicionar ingressos (use campo dataLimite conforme definido no modelo)
